...
import Language.SepPP.Syntax(Expr,EExpr(..),noTCast,unrollApp)
...
eval :: Integer -> Expr -> TCMonad EExpr
eval steps t = do
  t' <- erase t
  -- emit $ "Reducing" <++> t'
  evalErased steps t'

evalErased :: Integer -> EExpr -> TCMonad EExpr
evalErased steps t = reduce steps t (\_ tm -> return tm)

logReduce :: EExpr -> EExpr -> TCMonad ()
logReduce t t' = do
...
reduce' 0 t k = k 0 t
reduce' _steps EAbort _ = return EAbort
...
reduce' steps (ECase scrutinee alts) k = do
  ...
  where k' 0 t = k 0 (ECase t alts)
        ...
        substPat c _ [] = typeError  "Can't find pattern for constructor"
                               [(disp "The Constructor", disp c)]
        substPat c args (a:as) = do
          ((c',vs),body) <- unbind a
          if string2Name c' == c
            then do
...
reduce' steps t@(ERec binding) k = do
  ...
  k steps t'
reduce' steps (EPi s binding) k = do
...
-- | patMatch finds the matching branch for a constructor.
patMatch :: (EExpr, [EExpr]) -> [Bind (String, [Name EExpr]) EExpr] -> TCMonad EExpr
patMatch (c,_) [] = typeError "No Pattern Match" [(disp "The constructor", disp c)]
patMatch t@(ECon c,args) (b:bs) = do
  ((cname,vars),body) <- unbind b
  if string2Name cname == c
     then return $ substs (zip vars args) body
     else patMatch t bs
patMatch (c,_) _ = typeError "Can't pattern match with a non-constructor."           
                       [(disp "The constructor", disp c)]
